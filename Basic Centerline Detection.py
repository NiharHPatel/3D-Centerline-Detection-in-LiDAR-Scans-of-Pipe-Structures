...
output_dir = "/Users/niharpatel/Desktop/WNE UNIVERSITY/Capestone Project/Outputs"
...
# Function to process and save output
def process_ply_file(file_path, file_index):
    ...
    if len(slice_centroids) < 2:
        print(f"Not enough centroids to fit a spline for {file_path}. Skipping...")
        return
    ...
    # Step 5: Visualize and save the centerline in Open3D 
    ...
    # Save Open3D visualization
    vis = o3d.visualization.Visualizer()
    vis.create_window(visible=False)  # Render without opening window
    vis.add_geometry(filtered_pcd)
    vis.add_geometry(centerline_pcd)
    vis.poll_events()
    vis.update_renderer()
    open3d_screenshot_path = os.path.join(output_dir, f"file_{file_index}_open3d.png")
    vis.capture_screen_image(open3d_screenshot_path)
    vis.destroy_window()
    print(f"Saved Open3D visualization: {open3d_screenshot_path}")

    # Step 6: Plot and save the centerline in Matplotlib 
    ...
    matplotlib_screenshot_path = os.path.join(output_dir, f"file_{file_index}_matplotlib.png")
    plt.savefig(matplotlib_screenshot_path)
    print(f"Saved Matplotlib visualization: {matplotlib_screenshot_path}")
    plt.close()

# Run the process for all 9 PLY files
for index, file in enumerate(ply_files, start=1):
    process_ply_file(file, index)
