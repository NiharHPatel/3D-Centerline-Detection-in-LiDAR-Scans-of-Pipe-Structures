...
def preprocess_point_cloud(pcd):
    """
    Preprocess the point cloud by dynamically adjusting parameters.
    - Voxel downsampling (adaptive)
    - Outlier removal (adaptive)
    """
    voxel_size = np.linalg.norm(np.asarray(pcd.get_max_bound()) - np.asarray(pcd.get_min_bound())) * 0.005
...
def compute_centroids(z_slices):
    """
    Compute centroids for each Z-slice using clustering to adapt to curvature.
    """
    centroids = []
    for slice_points in z_slices:
        xy_points = slice_points[:, :2]
        if len(xy_points) >= 3:
            clustering = DBSCAN(eps=0.1, min_samples=3).fit(xy_points)
            cluster_labels = np.unique(clustering.labels_)
            for label in cluster_labels:
                if label != -1:
                    cluster_points = xy_points[clustering.labels_ == label]
...
def fit_centerline(centroids):
    ...
    """
    smooth_factor = max(10, len(centroids) * 0.1)  # Adjust smoothness based on data points
...
for file in ply_files:
    ...
    mean_deviation, distances = compute_deviation(centerline, pcd_filtered)
    print(f"Mean deviation for {file}: {mean_deviation:.4f}")
    ...
    # Histogram of deviation errors
    plt.figure(figsize=(8, 5))
    plt.hist(distances, bins=50, color='blue', alpha=0.7)
    plt.xlabel("Distance")
    plt.ylabel("Frequency")
    plt.title(f"Deviation from Centerline - {os.path.basename(file)}")
    plt.show()

    # Scatter plot of centroids with fitted centerline
