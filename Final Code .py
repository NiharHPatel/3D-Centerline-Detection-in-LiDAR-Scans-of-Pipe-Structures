...
def run_pipeline(file_path):
    ...
    if len(centers) < rosafallback_threshold:
        ...
        # === New: Global PCA Axis Correction for Fallback ===
        if len(centers) >= 4:
            pca = PCA(n_components=1)
            pca.fit(centers)
            axis = pca.components_[0]
            origin = pca.mean_
            projections = (centers - origin) @ axis
            centers = np.outer(projections, axis) + origin
            centers = fit_gpr_to_points(centers, num_points=spline_points)
        else:
            print("⚠️ Fallback points too few for axis correction.")
