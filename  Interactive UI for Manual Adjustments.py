...
def compute_centroids(z_slices, normals):
    """
    Compute centroids for each Z-slice using normal-based refinement.
    """
    centroids = []
    for i, slice_points in enumerate(z_slices):
        ...
        if len(xy_points) >= 3:
            ...
            # Ensure normal exists for this slice
            if i < len(normals):
                normal = np.mean(normals[i], axis=0)

                # Validate normal has correct shape
                if normal.shape == (3,):
                    refined_centroid = centroid + 0.05 * np.cross(normal, [0, 0, 1])
                else:
                    refined_centroid = centroid  # Fallback if normal is invalid
            else:
                refined_centroid = centroid  # Fallback if no normal data

            centroids.append([refined_centroid[0], refined_centroid[1], np.mean(slice_points[:, 2])])

    return np.array(centroids)

def fit_centerline(centroids, smooth_factor=50):
    """
...
for file in ply_files:
    ...
    centroids = compute_centroids(z_slices, normals)

    if len(centroids) < 3:
        print(f"Insufficient centroids for spline fitting in file: {file}")
        continue

    centerline = fit_centerline(centroids, smooth_factor=50)
    ...
    o3d.visualization.draw_geometries([pcd_filtered, centerline_pcd], window_name=f"3D Centerline - {os.path.basename(file)}")

    # Plot centerline with centroids
    fig = plt.figure(figsize=(8, 6))
    ax = fig.add_subplot(111, projection="3d")
    ax.scatter(centroids[:, 0], centroids[:, 1], centroids[:, 2], c="green", label="Refined Centroids")
    ax.plot(centerline[:, 0], centerline[:, 1], centerline[:, 2], c="red", label="Refined Centerline")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
    ax.legend()
