...
def compute_centroids(z_slices):
    """
    Compute centroids for each Z-slice and apply DBSCAN clustering for better accuracy.
    """
    centroids = []
    ...
    if len(centroids) > 5:  # Only apply if there are enough points
        clustering = DBSCAN(eps=0.5, min_samples=3).fit(centroids)
        centroids = centroids[clustering.labels_ != -1]  # Remove outliers

    return centroids

def adaptive_spline_smoothing(centroids):
    """
    ...
    total_curvature = np.sum(np.abs(np.diff(centroids[:, :2], axis=0)))  # Approximate curvature
    smooth_factor = max(50, min(500, total_curvature * 5))  # Adjust smoothness dynamically
...
for file in ply_files:
    ...
    centerline = adaptive_spline_smoothing(centroids)
